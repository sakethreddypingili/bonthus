...
const titles = {
"/": { title:"Overview", subtitle:"Welcome back, here's what's happening today" },
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative" ref={notificationsRef}>
          <button 
            ...
            className={`relative p-2.5 rounded-xl  ${showNotifications ? 'bg-black text-white shadow-lg scale-105' : 'text-gray-500 hover:bg-gray-100 hover:text-black'}`}
          >
          ...
          {showNotifications && (
            <div className="absolute right-0 mt-3 w-80 bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden z-50">
              <div className="p-4 border-b border-gray-100 flex items-center justify-between">
                ...
                <button onClick={() => setShowNotifications(false)} className="text-gray-400 hover:text-black"><X size={16} strokeWidth={3} /></button>
              </div>
        ...
        <div className="relative" ref={profileRef}>
          <button 
            ...
            className={`flex items-center gap-3 pl-2 pr-4 py-1.5 rounded-xl  ${showProfileMenu ? 'bg-black text-white shadow-lg' : 'hover:bg-gray-50 border border-transparent hover:border-gray-100'}`}
          >
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-[11px] font-black uppercase  ${showProfileMenu ? 'bg-white text-black' : 'bg-black text-white'}`}>
              {userProfile?.name ? userProfile.name.split(' ').map(n => n[0]).join('').substring(0,2) :"SA"}
            </div>
            <div className="hidden md:block text-left">
              <div className={`text-[11px] font-black uppercase tracking-tight  ${showProfileMenu ? 'text-white' : 'text-black'}`}>{userProfile?.name ||"Store Admin"}</div>
              <div className={`text-[9px] font-bold uppercase tracking-wider  ${showProfileMenu ? 'text-gray-400' : 'text-gray-400'}`}>{userProfile?.role ||"Administrator"}</div>
            </div>
            <ChevronDown size={14} strokeWidth={3} className={`${showProfileMenu ? 'rotate-180 text-white' : 'text-gray-300'}`} />
          </button>
            ...
            <div className="absolute right-0 mt-3 w-64 bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden z-50">
              ...
              <div className="p-2 space-y-1">
                <button 
                  ...
                  className="w-full flex items-center gap-3 px-4 py-2.5 text-[11px] font-black text-gray-600 uppercase tracking-widest hover:bg-black hover:text-white rounded-xl  group"
                >
              ...
              <div className="p-2 border-t border-gray-100">
                <button
                  ...
                  className="w-full flex items-center gap-3 px-4 py-2.5 text-[11px] font-black text-red-600 uppercase tracking-widest hover:bg-red-600 hover:text-white rounded-xl  group"
                >

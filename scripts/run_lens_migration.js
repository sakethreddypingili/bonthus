...
const migrationPath = path.join(repoRoot, 'admin', 'schema', 'migration_lens_catalog.sql');
const seedPath = path.join(repoRoot, 'admin', 'schema', 'seed_lens_catalog.sql');
...
async function main() {
  ...
  await client.query('TRUNCATE TABLE public.lens_catalog RESTART IDENTITY CASCADE;');

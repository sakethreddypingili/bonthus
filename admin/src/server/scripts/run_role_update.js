...
const SQL = fs.readFileSync(path.resolve(__dirname, '../server/database/update_roles_stores.sql'), 'utf8');

async function upgrade() {
    console.log(`🔧 Applying Roles & Stores Upgrade to: ${projectRef}`);
    try {
        ...
        if (res.ok) console.log('✅ Custom DB Update Successful.');
        else console.error('❌ Custom DB Update failed:', await res.text());
    } catch (e) {

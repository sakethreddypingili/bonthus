...
export default function Repairs({ userProfile }) {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const res = await fetch(`/api/customers/search?phone=${encodeURIComponent(phone.trim())}`);
  ...
  const handleSubmitRepair = async (e) => {
    ...
    try {
      ...
      if (!targetStoreId || targetStoreId === "00000000-0000-0000-0000-000000000000") {
        const { data: storeList } = await supabase.from("stores").select("id").limit(1);
        if (storeList && storeList.length > 0) {
          targetStoreId = storeList[0].id;
        } else {
          throw new Error("No valid store exists in the system to link the repair to.");
        }
      }
    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-fast-slide pb-20">
      ...
      <div>
        <h1 className="text-3xl font-black text-black tracking-tighter uppercase mb-2 flex items-center gap-2">
          <Wrench className="text-black" size={28} /> Repair Order Wizard
        </h1>
      ...
      <div className="grid grid-cols-3 gap-2">
        {[
          { num: 1, label: "Customer Lookup" },
          { num: 2, label: "Specifications" },
          { num: 3, label: "Financials & Submit" }
        ].map((s) => (
          <div
            key={s.num}
          ...
          >
            <div className="text-[10px] font-black uppercase tracking-widest">Step {s.num}</div>
            <div className="text-xs font-bold mt-1 uppercase tracking-tight">{s.label}</div>
          </div>
        ))}
      </div>
